...
int main()
{
  ...
  int focus_x = 55, focus_y = 40, focus_t=2;
  ...
  int ns = 32;
  float left_edges[ns];
  for (int i = 0; i < ns - 1; ++i) left_edges[i] = 1e-6 * pow(10, -2 + i * .125);  //TODO * si::metres
...
//  {
    ...
    for (int i = 0; i < ns; ++i)
    {
      ...
      focus[left_edges[i] / 1e-6] = sum(tmp(
