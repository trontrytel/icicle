...
{
  // <nested class>
  private: class slv_halo : public slv<unit, real_t>
  {
    private: slv_parallel_distmem<unit, real_t, shrdmem_class> *nghbr;
    private: int peer, cnt;
    private: auto_ptr<Array<quantity<unit, real_t>, 3> > ibuf, obuf;
    private: Range ixr, oxr, yr, zr;

    public: slv_halo(slv_parallel_distmem<unit, real_t, shrdmem_class> *nghbr, int peer,
      const Range &ixr, const Range &oxr, const Range &yr, const Range &zr
    )
      : nghbr(nghbr), peer(peer), ixr(ixr), oxr(oxr), yr(yr), zr(zr)
    { 
      ibuf.reset(new Array<quantity<unit, real_t>, 3>(ixr, yr, zr));
      obuf.reset(new Array<quantity<unit, real_t>, 3>(oxr, yr, zr));
      cnt = ibuf->cols() * ibuf->rows() * ibuf->depth();
    }

    public: Array<quantity<unit, real_t>, 3> data(int n, const Range &i, const Range &j, const Range &k) 
    { 
      return (*ibuf)(i, j, k); 
    }
  ...
  private: auto_ptr<Array<quantity<unit, real_t>, 3> > libuf, ribuf, lobuf, robuf;
  ...
  private: virtual void sndrcv(int peer, int cnt, quantity<unit, real_t> *ibuf, quantity<unit, real_t> *obuf) = 0;
};
