...
void init(Gnuplot &gp, const std::string &file, const int &ny, const int &nx)
{
  ...
  gp << "set xtics out scale .5 rotate by 60 ('0' 0, '0.3' 300/dx, '0.6' 600/dx, '0.9' 900/dx, '1.2' 1200/dx, '1.5' 1500/dx)\n"; 
  gp << "set xlabel 'x [km]'\n";
  gp << "set ytics out scale .5 rotate by 60 ('0' 0, '0.3' 300/dy, '0.6' 600/dy, '0.9' 900/dy, '1.2' 1200/dy, '1.5' 1500/dy)\n"; 
  gp << "set ylabel 'y [km]'\n";
  gp << "set output '" << file << "'\n";
  gp << "set grid\n";
...
void plot(Gnuplot &gp, const data_t &data)
{
  ...
  gp << "splot '-' binary" << gp.binfmt(tmp) << " origin=(.5,.5,0) with image failsafe notitle\n";
