...
int main(int ac, char** av)
{
  ...
  for (int at = 0; at < n["t"]; ++at) // TODO: mark what time does it actually mean!
  {
	...
	// cloud water content
	//                                                         rho_w  kg2g
	auto tmp = h5load(h5, "rw_rng000_mom3", at) * 4./3 * 3.14 * 1e3 * 1e3;
	...
	// rain water content
	//                                                         rho_w  kg2g
	auto tmp = h5load(h5, "rw_rng001_mom3", at) * 4./3 * 3.14 * 1e3 * 1e3;
	gp << "set logscale cb\n";
	...
	auto tmp = 1e-6 * h5load(h5, "rw_rng000_mom0", at);
	...
	auto tmp = 1e-6 * h5load(h5, "rw_rng001_mom0", at);
	...
	gp << "unset logscale cb\n";
      ...
      {
	// effective radius
	auto r_eff = h5load(h5, "rw_rng000_mom3", at) / h5load(h5, "rw_rng000_mom2", at) * 1e6;
	...
	blitz::Array<float, 2> tmp(h5load(h5, "rw_rng002_mom0", at));
